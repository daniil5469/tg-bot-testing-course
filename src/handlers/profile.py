...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "create_profile":
        ...
        ) if profile else "⚠️ Profile not found. Use Create Profile to create one."
        await query.edit_message_text(msg, reply_markup=BACK_BUTTON)
        return ConversationHandler.END

    elif data == "delete_profile":
        success = user_store.delete_user_profile(chat_id)
        text = "🗑️ Your profile has been deleted successfully." if success else "⚠️ No profile found to delete."
    ...
    await query.edit_message_text("⚠️ Unknown option selected.", reply_markup=BACK_BUTTON)
    return ConversationHandler.END
...
async def view_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"🎂 Date of Birth: {profile.get('dob')}\n"
        f"📍 Location: {profile.get('location')}"
    ) if profile else "⚠️ Profile not found. Use /start to create one."
    await update.message.reply_text(msg)

async def delete_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    success = user_store.delete_user_profile(chat_id)
    msg = "🗑️ Your profile has been deleted." if success else "⚠️ No profile found to delete."
...
async def unknown_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⚠️ Unknown command. Use /start to see available options.")
